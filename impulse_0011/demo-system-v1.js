...
class DemoSystemV1 {
  ...
  constructor(analyze_audio = false) {
    ...
    this.analyze_audio_ready      = false                  ;
    this.fft_size                 = 512                    ;
  ...
  play() {
    if (this.initialized && !this.playing) {
      if (this.never_played) {
        ...
        // We have to wait initialize the AudioContext until the user interacted with audio element
  ...
  async init_shaders() {
    for (const key in all_scenes) {
      ...
      if (!this.gl.getProgramParameter(scene.shaderProgram, this.gl.LINK_STATUS)) {
        console.log("Unable to initialize the shader program: " + key);
      }
